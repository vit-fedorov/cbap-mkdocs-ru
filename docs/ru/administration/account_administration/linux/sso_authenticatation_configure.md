...
_![Добавление машины с экземпляром ПО в список доверенных узлов на контроллере домена](img/sso_authenticatation_configure_make_host_machine_trusted.png)_
...
_![Настройка свойств сервисного аккаунта для NGINX](img/Kerberos_Rocky+Astra_authuser_account_config.png)_
...
5. [Настройте машину `<linuxHost>`](##настройка-машины-linuxhost-с-экземпляром-по-с-экземпляром-по) с экземпляром ПО.
...
1. [Настройте машину `<linuxHost>`](##настройка-машины-linuxhost-с-экземпляром-по-с-экземпляром-по) с экземпляром ПО.
...
2. Отредактировать файл конфигурации `pam_winbind.conf` согласно приведённому ниже примеру:
