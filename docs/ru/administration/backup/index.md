...
Для обеспечения высокой доступности и отказоустойчивости системы **{{ productName }}** важно надлежащим образом организовать резервное копирование.
...
Предусмотрен [журнал сеансов резервного копирования](#просмотр-списка-сеансов-резервного-копирования).
...
Следуя данным рекомендациям, вы сможете обеспечить высокую доступность и отказоустойчивость **{{ productName }}**, непрерывность бизнес-процессов, минимизировать показатели RPO и RTO и снизить риски.
...
- **Репликация — непрерывно**:
    - Репликация данных средствами Apache Ignite, Elasticsearch, Kafka и файловой системы NFS, развёрнутых в отказоустойчивой конфигурации (с зеркалированием).
...
- **Во внешнем репозитории**:
    - Хранение копий на внешнем репозитории обеспечивает дополнительную защиту данных и позволяет восстановить систему в случае катастрофического сбоя.
