...
10. Для восстановления резервной копии используйте следующие параметры из YML-файла конфигурации экземпляра ПО `/usr/share/comindware/configs/instance/<instanceName>.yml`:
    ...
    !!! note "Восстановление резервной копии на другом экземпляре ПО"
        ...
        При создании экземпляра ПО база данных связывается с именем, которое указано в параметре `db.name` конфигурационного файла.

        Если вы восстанавливаете резервную копию на новом экземпляре ПО, необходимо убедиться, что в YML-файле конфигурации задано имя базы данных того экземпляра ПО, на котором была создана резервная копия.
