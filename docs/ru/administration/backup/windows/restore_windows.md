...
2. Если откроется страница настройки подключения к службе {{ openSearchVariants }}:
...
3. Если откроется страница инициализации данных в {{ openSearchVariants }}, нажмите кнопку «**Обновить**».
...
6. Откройте подключение к серверу {{ openSearchVariants }}.
7. Удостоверьтесь, что корректно указаны **префикс индекса** и **URL подключения** к серверу {{ openSearchVariants }}.
...
10. Откройте страницу **[«Администрирование» — «Резервное копирование»][backup_configure]**.
...
- _[Резервное копирование. Настройка и запуск {{ productName }}][backup_configure]_
