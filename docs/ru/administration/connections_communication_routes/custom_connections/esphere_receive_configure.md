...
3. Настройте [шаблон](#настройка-шаблона-записи) для данных электронных документов.
...
- Для типа сообщения «**Поиск документов**»:
    ...
    - **Описание** — укажите описание для поиска документа в системе «СФЕРА Курьер».
...
4. В настройках действия добавьте следующие переменные:
    ...
    - _Content_ со значением **N3**:
...

        {
            ...
            session:context var:Message ?message.
            # Получаем из ?message документ и помещаем в ?document
            ?message var:Content ?document.
            # Из ?document получаем содержимое документа и возвращаем его
            ?document var:Content ?value.
        }
        ```
...
5. Добавьте действие «**Выполнить по условиям**» со следующими параметрами:
    ...
    - **Выражение: N3**
        ...
        @prefix assert: <http://comindware.com/logics/assert#>.
        {
            ...
            # Получаем из ?message ID документа и помещаем его в ?docId
            ?message variable:Id ?docId.
...
6. Внутрь действия «**Выполнить по условиям**» поместите действие «**Создать запись**» и выберите в качестве целевого шаблона записи шаблон _«Реестр документов из СФЕРА Курьер»_.
...
{
    # Получаем значение локальной переменной Message
    # и помещаем в ?message
    session:context variable:Message ?message.
    # Получаем из ?message ID документа и помещаем его в ?docId
    ?message variable:Id ?docId.
