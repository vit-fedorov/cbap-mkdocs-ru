...
1. Настройте пользовательское [подключение](#настройка-подключения-для-получения-id-документов) типа «**Получение сообщений из системы «СФЕРА Курьер**».
...
3. Настройте пользовательское [подключение](#настройка-подключения-для-отправки-запроса-к-сфера-курьер) типа «**Отправка сообщений в систему «СФЕРА Курьер**».
...
!!! tip "Совет"
    ...
    Из-за особенностей API «СФЕРА Курьер» при получении сообщений приходят не все атрибуты электронного документа. Чтобы получить недостающие атрибуты, требуется отправить дополнительный запрос с ID требуемого документа в «СФЕРА Курьер» следующим образом:

    1. Настройте подключение типа «**Отправка сообщений в систему «СФЕРА Курьер**».
    2. Настройте путь передачи данных «**Отправка сообщений в «СФЕРА Курьер**» типа «**Получить документ**».
    3. Отправьте запрос с ID документа с помощью сценария.
    
    См. _«[Настройка подключения для отправки запроса к «СФЕРА Курьер](#настройка-подключения-для-отправки-запроса-к-сфера-курьер)»_.
...
## Настройка подключения для отправки запроса к «СФЕРА Курьер»
...
## Настройка пути передачи данных для отправки запроса к «СФЕРА Курьер»
...
1. Выберите **тип сообщения**:
    ...
    В примере используется тип сообщения «**Получить документ**».
...
1. Отправьте документ пользователю системы «СФЕРА Курьер», API которого используется при [подключении](#настройка-подключения-для-получения-id-документов).
