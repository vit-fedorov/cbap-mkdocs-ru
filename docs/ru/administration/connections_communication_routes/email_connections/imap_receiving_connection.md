...
В **{{ productName }}** можно настроить получение эл. почты. При этом данные писем можно обрабатывать для использования внутри системы.
...
Здесь представлены общие инструкции по настройке подключения к почтовому серверу по протоколу IMAP или Exchange, пути передачи данных и сценария для получения эл. писем.

## Порядок настройки получения эл. писем из сценария

1. Настройте [подключение](#настройка-подключения-для-получения-элпочты-из-сценария) к почтовому серверу типа «**Получение эл. почты через IMAP**» или «**Получение сообщений через Exchange**».
2. Настройте [путь передачи данных](#настройка-пути-передачи-данных-для-получение-элпочты-из-сценария) типа «**Получение эл. почты через IMAP**» или «**Получение сообщений через Exchange**», использующий созданное подключение.
3. Настройте [шаблон](#настройка-шаблона-записи) с данными из эл. писем.
4. Настройте [сценарий](#настройка-сценария), который будет обрабатывать данные писем, полученных посредством настроенных подключения и пути передачи данных.
...
Для каждого типа сообщения имеются готовые атрибуты, список которых хранится в таблицах «**Запрос**», «**Ответ**» и «**Ответ с ошибкой**». Также можно задать собственные атрибуты сообщения.
...
С помощью таблиц «**Ответ**» и «**Ответ с ошибкой**» можно настроить атрибуты сообщения, которые будет отправлять сервер **{{ productName }}** после выполнения **запроса** на чтение писем:

- Если серверу **{{ productName }}** удалось прочитать письмо, то он отправит атрибуты сообщения из таблицы «**Ответ**».
- Если серверу **{{ productName }}** не удалось прочитать письмо, то он отправит  атрибуты сообщения из таблицы «**Ответ с ошибкой**».

Значения данных атрибутов следует заполнить при помощи сценария.
...
8. Добавьте операцию «**Добавить**» для атрибута _«Вложения»_ и укажите выражение N3:
