...
1. [Настройте подключение](#настройка-подключения) типа «**Получение эл. почты в процессе**» или «».
...
2. Откройте двойным нажатием в списке или создайте **подключение к электронной почте** типа «**Получение эл. почты в процессе**» или **подключение REST и OData** типа «**Получение веб-запросов**».
3. Настройте подключение:
    ...
    - для **получения веб-запросов**:
        ...
        - **Заголовок страницы** — укажите заголовок, который будет отображаться на странице получения веб-запросов.
        - **Тело страницы** — укажите сообщение, которое будет отображаться на странице получения веб-запросов.

4. Нажмите кнопку «**Создать**».

## Настройка пути передачи данных {: .pageBreakBefore }

Путь передачи данных для получения эл. почты в процессе служит для преобразования и передачи данных из эл. письма или HTTP-запроса в **{{ productName }}**.
...
- **Метод парсинга ответа** — выберите метод анализа данных HTTP-запроса:
    - **Не преобразовывать**;
    - **XML**;
    - **JSON**;
    - **Строка запроса**.
...
На вкладке «**Атрибуты сообщения**» заполните таблицу соответствия атрибутов сообщения с атрибутами электронного письма или HTTP-запроса.
...
!!! tip "Извлечение значений атрибутов из HTTP-запроса"

    Путь передачи данных для получения эл. почты в процессе служит также для передачи данных из HTTP-запросов в **{{ productName }}**.

    - На вкладке «**Атрибуты сообщения**» пути передачи данных задайте набор атрибутов, значения которых требуется извлечь из HTTP-запроса. Для этого создайте атрибуты и сопоставьте их с полями HTTP-запроса.

        - Например, поместите дату запроса в атрибут _Date_:

            - **Название:** _Дата_
            - **Системное имя:** _Date_
            - **Тип данных: дата и время**
            - Укажите выражение для анализа данных HTTP-запроса в зависимости от его типа.
...
!!! note "Примечание"
    ...
    Для HTTP-запросов в зависимости от выбранного **метода парсинга ответа** на месте столбца «**Поле сообщения**» будет отображаться столбец для ввода выражения для анализа полученного запроса.
