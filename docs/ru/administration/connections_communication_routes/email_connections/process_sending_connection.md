...
## Введение {: #process_sending_connection_intro }
...
## Порядок настройки отправки эл. писем {: #process_sending_connection_setup_order }

1. [Настройте подключение](#process_sending_connection_smtp_configure) типа «**Отправка эл.&nbsp;почты из процесса**».
2. Для отправки писем из события на диаграмме процесса:

    - [Настройте путь передачи данных](#process_sending_connection_communication_route_configure) типа «**Отправка эл.&nbsp;почты из процесса**».
    - [Настройте событие-отправку сообщения](#process_sending_connection_event_configure) на диаграмме процесса.
...
## Настройка подключения типа «Отправка эл.&nbsp;почты из процесса» {: #process_sending_connection_smtp_configure .pageBreakBefore }
...
## Настройка пути передачи данных типа «Отправка эл.&nbsp;почты из процесса» {: #process_sending_connection_communication_route_configure .pageBreakBefore }
...
### Основные свойства {: #process_sending_connection_basic_properties .pageBreakBefore }
...
### Атрибуты сообщения {: #process_sending_connection_message_attributes .pageBreakBefore }

На вкладке «**Атрибуты сообщения**» настройте атрибуты, которые будут использоваться для передачи данных из процесса в эл. письмо. Эти атрибуты необходимо указать на вкладке «**[Свойства сообщения](#process_sending_connection_message_properties)**» пути передачи данных, а также необходимо присвоить им значения на вкладке «**Данные сообщения**» в свойствах [события-отправки сообщения](#process_sending_connection_event_configure).
...
_![Атрибуты сообщения — путь передачи данных для отправки эл.&nbsp;почты из процесса](outgoing_email_configure_message_attributes.png)_

### Свойства сообщения {: #process_sending_connection_message_properties .pageBreakBefore }

На вкладке «**Свойства сообщения**» настройте шаблон эл. письма с использованием атрибутов с вкладки «**Атрибуты сообщения**».

- **Список атрибутов, которые можно использовать при написании эл.&nbsp;письма**, содержит атрибуты с вкладки «**[Атрибуты сообщения](#process_sending_connection_message_attributes)**».
...
- **Электронное письмо** содержит шаблон, в полях которого можно ввести статические значения и системные имена **атрибутов сообщения**. В шаблоне эл. письма предусмотрены перечисленные ниже поля.
    - **Адрес отправителя** — в этом поле можно указать адрес эл.&nbsp;почты отправителя, отличный от адреса отправителя, указанного в [свойствах подключения](#process_sending_connection_smtp_configure). Если оставить это поле пустым, то будет использоваться адрес отправителя, указанный в свойствах подключения.
    - **Имя отправителя** — в этом поле можно указать имя отправителя, отличный от адреса отправителя, указанного в [свойствах подключения](#process_sending_connection_smtp_configure).

        !!! note "Примечание"

            Если **адрес** или **имя отправителя** не совпадает с адресом и именем, указанным в учётной записи, используемой на [почтовом сервере для отправки почты](#process_sending_connection_smtp_configure), сервер должен поддерживать замену данных отправителя. В противном случае, почта не будет отправляться.
...
## Настройка события-отправки сообщения {: #process_sending_connection_event_configure .pageBreakBefore }
...
4. На вкладке «**Дополнительные**» выберите путь передачи данных.
    ...
    - С помощью раскрывающегося списка «**Использовать путь передачи данных**» выберите или создайте **[путь передачи данных](#process_sending_connection_communication_route_configure)** типа «**Отправка эл.&nbsp;почты из процесса**».
...
5. На вкладке «**Данные сообщения**» сопоставьте **[атрибуты сообщения](#process_sending_connection_message_attributes)** с атрибутами **шаблона записи**.
    ...
    - Значения указанных атрибутов **шаблона записи** будут присвоены **атрибутам сообщения** и переданы в [эл.&nbsp;письмо](#process_sending_connection_message_properties).
