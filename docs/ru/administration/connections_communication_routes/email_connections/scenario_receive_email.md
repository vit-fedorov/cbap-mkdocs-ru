...
kbTitle: Получение эл.&nbsp;почты с помощью сценариев через IMAP и Exchange. Настройка подключения, пути передачи данных и сценария
kbId: 4693
tags:
    - сценарий
    - получение эл.&nbsp;почты
...
## Введение {: #scenario_receive_email_intro }
...
## Порядок настройки получения эл. писем из сценария {: #scenario_receive_email_setup_order }

1. Настройте [подключение](#scenario_receive_email_connection_configure) к почтовому серверу типа «**Получение эл.&nbsp;почты через IMAP**» или «**Получение сообщений через Exchange**».
2. Настройте [путь передачи данных](#scenario_receive_email_communication_route_configure) типа «**Получение эл.&nbsp;почты через IMAP**» или «**Получение сообщений через Exchange**», использующий созданное подключение.
3. Настройте [шаблон записи](#scenario_receive_email_template_configure) для хранения данных из эл. писем.
4. Настройте [сценарий](#scenario_receive_email_scenario_configure), который будет обрабатывать и помещать в шаблон записи данные из писем, полученных посредством настроенных подключения и пути передачи данных.

## Настройка подключения к почтовому серверу для получения почты с помощью сценария {: #scenario_receive_email_connection_configure }
...
## Настройка пути передачи данных для получения почты с помощью сценария {: #scenario_receive_email_communication_route_configure .pageBreakBefore }
...
4. Настройте свойства пути передачи данных на следующих вкладках:
    ...
    - [**Основные свойства**](#scenario_receive_email_general_properties)
    - [**Атрибуты сообщений**](#scenario_receive_email_message_attributes)
...
- **Подключение** — выберите [подключение для получения эл.&nbsp;почты из сценариев через SMTP или Exchange](#scenario_receive_email_connection_configure).
...
### Атрибуты сообщений {: #scenario_receive_email_message_attributes .pageBreakBefore }
...
!!! tip "Извлечение значений атрибутов из эл. письма"
    ...
    - На вкладке «[**Атрибуты сообщений**](#scenario_receive_email_message_attributes)» пути передачи данных настройте набор атрибутов, значения которых требуется извлечь из письма.
    ...
    - С помощью [сценария](#scenario_receive_email_scenario_configure) передайте значения атрибутов сообщения, содержащие данные из эл. письма, в шаблон записи. Например, передайте текст и тему письма из атрибутов сообщения _HtmlBody_ и _Subject_ в атрибуты шаблона записи _«Текст письма»_ и _«Тема письма»_.

1. Выберите **тип сообщения**:

    - **Получение эл.&nbsp;почты** — для [IMAP и Exchange](#scenario_receive_email_imap_exchange_attributes);
    - **Получение события календаря** — для [Exchange](#scenario_receive_email_calendar_attributes).
...
2. В таблицах «**Запрос**», «**Ответ**» и «**Ответ с ошибкой**» отобразятся готовые атрибуты сообщений, соответствующие выбранному **типу сообщения**:
    ...
    - [получение эл.&nbsp;почты через IMAP и Exchange](#scenario_receive_email_imap_exchange_attributes);
    - [получение событий календаря через Exchange](#scenario_receive_email_calendar_attributes);
    - [ответ почтовому серверу](#scenario_receive_email_response_attributes).

3. При необходимости [настройте собственные атрибуты сообщений](#scenario_receive_email_custom_attribute)

#### Атрибуты для получения эл.&nbsp;почты через IMAP и Exchange {: #scenario_receive_email_imap_exchange_attributes }
...
#### Атрибуты для получения событий календаря через Exchange {: #scenario_receive_email_calendar_attributes }
...
#### Атрибуты для ответа почтовому серверу {: #scenario_receive_email_response_attributes }
...
Значения атрибутам **ответа** и **ответа с ошибкой** необходимо присвоить с помощью действия «**Изменить значения переменных**» в [сценарии](#scenario_receive_email_scenario_configure).
...
#### Настройка собственного атрибута сообщения {: #scenario_receive_email_custom_attribute .pageBreakBefore }
...
## Настройка шаблона записи и сценария для получения эл. писем {: #scenario_receive_email_template_and_scenario .pageBreakBefore }
...
### Настройка шаблона записи {: #scenario_receive_email_template_configure }
...
### Настройка сценария {: #scenario_receive_email_scenario_configure .pageBreakBefore }
...
2. Откройте автоматически созданное событие «**Нажатие кнопки**» и настройте его следующим образом:
    ...
    - **Подключение:** выберите [подключение для получения эл.&nbsp;почты с помощью сценария](#scenario_receive_email_connection_configure);
    - **Путь передачи данных:** выберите [путь передачи данных эл.&nbsp;почты с помощью сценария](#scenario_receive_email_communication_route_configure);
...
9. На вкладке «**Основные**» добавьте в таблицу атрибут _«Вложения»_:
...
## Тестирование {: #scenario_receive_email_testing .pageBreakBefore }

1. В почтовом клиенте создайте письмо с вложениями и отправьте его на адрес эл.&nbsp;почты, указанный в [подключении](#scenario_receive_email_connection_configure).
2. Перейдите к списку экземпляров [шаблона _«Реестр документов»_](#scenario_receive_email_template_configure).
3. В списке экземпляров откройте новую запись с временем создания несколько позже отправки письма на шаге 1.
    ...
    !!! tip "Совет"
        ...
        Время обработки входящих писем зависит от **интервала опроса**, заданного в [свойствах подключения к почтовому серверу](#scenario_receive_email_connection_configure), а также от загруженности почтового сервера и **{{ productName }}**.
