...
# Отправка эл. почты через SMTP. Настройка подключения

## Введение

Подключения типа «**Отправка эл. почты через SMTP**» используются в [сценариях](scenario_configure.md). В [процессах]({{ kbArticleURLPrefix }}2360) используются подключения типа «**[Отправка эл. почты из процесса](process_sending_connection.md)**».
