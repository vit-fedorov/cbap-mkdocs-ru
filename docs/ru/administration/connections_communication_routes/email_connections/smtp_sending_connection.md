...
В **{{ productName }}** можно настроить отправку эл. почты из сценариев. При этом в письма можно включать произвольные данные, в том числе вложенные файлы.

Для отправки эл. почты с помощью [сценариев][scenarios] используются подключения типов «**Отправка эл. почты через SMTP**» и «**Отправка эл. почты через Exchange**». В [процессах][process_diagram] используются подключения типа «**[Отправка эл. почты из процесса][process_sending_connection]**».
...
4. Настройте сценарий, который будет отправлять письма посредством настроенных подключения и пути передачи данных.
