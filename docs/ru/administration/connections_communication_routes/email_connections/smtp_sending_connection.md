...
В **{{ productName }}** можно настроить отправку эл. почты из сценариев. Данный метод позволяет включать в письма произвольные данные, в том числе вложенные файлы.
...
4. Настройте сценарий, который будет отправлять письма с помощью созданного подключения и пути передачи данных.
...
На вкладке «**Атрибуты сообщения**» настройте атрибуты, которые будут использоваться для передачи данных в эл. письмо. Значения атрибутов сообщения необходимо заполнить с помощью [сценария](#настройка-сценария). Также можно создать дополнительные атрибуты сообщения, которые следует подставить в содержимое письма с помощью вкладки [«**Интеграция**»](#интеграция).

В таблице «**Запрос**» представлены готовые атрибуты в виде массивов объектов:
...
В таблицу «**Запрос**» можно добавить собственные атрибуты, например:
...
На вкладке «**Интеграции**» настройте дополнительные свойства эл. письма, заполнив их статическими значениями или [**атрибутами сообщения**](#атрибуты-сообщения).

!!! tip "Текстовые заменители"
    ...
    - В текстовые поля (шаблон темы, шаблон текста сообщения, получатели, копия, скрытая копия) можно вводить произвольный текст и подставлять **атрибуты сообщений**.
    ...
    !!! warning "Внимание!"
        ...
        К атрибутам сообщения типа «**Объект**» обращаться через текстовые заменители нельзя.
...
- **Дополнительные заголовки** — введите служебные заголовки письма с помощью строковых литералов или заменителей:

    - Имя заголовка — строка (заменители не поддерживаются), например:
    ...
    - Значение заголовка — строка или заменитель, например: 
...
- **Получатели**, **Копия**, **Скрытая копия** — укажите адресатов письма с помощью строковых литералов или заменителей, например:
    - **Ф. И. О.**:
...
- **Прикреплённые файлы**
    ...
    - **Название документа** — введите строковый литерал с именем файла, включая расширение (заменители не поддерживаются). Например:

    ```cs
    example.txt
    ```

    - **Содержимое документа** — введите строку с содержимым файла в формате Base64 (заменители не поддерживаются). Например:
    ...
    !!! question "Base64"
        ...
        **Base64** — это стандарт кодирования двоичных данных при помощи только 64 символов ASCII. Эта система широко используется для представления бинарных файлов в тексте письма. Например, строка:
...
4. В свойствах действия «**Изменить значения переменных**» настройте перечисленные ниже переменные.
    ...
    !!! warning "Сопоставление переменных-объектов с атрибутами сообщения"
        ...
        - В таблице отобразится строка для дочерней переменной.
