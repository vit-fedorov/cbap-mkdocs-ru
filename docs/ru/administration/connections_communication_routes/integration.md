...
**{{ productName }}** включает в себя преднастроенные интеграции, в том числе:
...
- Отправка и получение данных из системы «СФЕРА Курьер»;
- Отправка и получение данных из СУБД.

Помимо этого **{{ productName }}** позволяет настроить интеграцию с рядом внешних систем, в том числе:
...
<!-- возможно, сдвинуть ближе к концу -->
...
<!-- только про одата, дать ссылку и заимствовать определения -->
...
<!-- интегрировать и убрать дубликаты с руководство по апи, при необходимости заимствовать инклюдами -->
...
<!-- структурировать и сгруппировать тезисы по темам так, чтобы постепенно раскрывался порядок: определение, порядок настройки, примеры -->
...
Это нужно обязательно знать, потому что при настройке новой интеграции в Comindware application platform нужно будет выбирать тип интеграции: «Импорт» или «Экспорт».
