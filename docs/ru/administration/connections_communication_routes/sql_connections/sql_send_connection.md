...
## Введение {: #sql_send_connection_introduction }
...
## Прикладная задача {: #sql_send_connection_applied_task .pageBreakBefore }
...
### Исходные данные {: #sql_send_connection_initial_data }
...
## Подготовка сервера СУБД {: #sql_send_connection_db_server_preparation .pageBreakBefore }

Перед подключением из **{{ productName }}** необходимо подготовить конфигурацию сервера СУБД, как указано ниже.

### Настройка конфигурации сервера PostgreSQL {: #sql_send_connection_postgresql_server_configuration }
...
## Создание подключения к СУБД {: #sql_send_connection_create_db_connection .pageBreakBefore }
...
## Создание пути для обмена данными с СУБД {: #sql_send_connection_create_communication_route .pageBreakBefore }
...
## Создание кнопки для отправки SQL-запроса в СУБД {: #sql_send_connection_create_button .pageBreakBefore }
...
## Создание сценария для отправки SQL-запроса и получения данных из СУБД {: #sql_send_connection_create_scenario .pageBreakBefore }
...
10. Добавьте действие «**Изменить значения переменных**» и настройте его свойства:
    ...
    - Нажимайте кнопку «**Создать**» над таблицей переменных и добавьте переменные:
        ...
        - `Filter` — не указывайте **значение** (объявление объекта). Установите флажок рядом c этой переменной и создайте дочерние переменные:
            ...
            - `Value` — в поле «**Значение**» выберите **атрибут** _«Код страны»_.
...
## Тестирование сценария {: #sql_send_connection_scenario_testing }

1. Создайте запись в шаблоне _«Страны»_ и заполните поле _«Код страны»._
2. Нажмите кнопку _«Получить города из СУБД»_.
3. [Настроенный для кнопки сценарий](#sql_send_connection_create_scenario) должен создать в шаблоне записи _«Города»_ записи с названиями городов, полученных из внешней БД по указанному коду страны.
