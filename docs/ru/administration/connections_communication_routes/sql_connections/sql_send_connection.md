---
title: Внешняя СУБД. Отправка SQL-запроса SELECT
kbId: 2498
---
...
11.  Если проверка соединения не выдала ошибок, нажмите кнопку «**Сохранить**».
    ...
    _![Сообщение об успешной проверке соединения](img/sql_send_connection_check_passed.png)_
...
6. В отобразившемся конструкторе сценария нажмите кнопку изменить на блоке «**Нажатие кнопки**».  
    ...
    _![Переход к свойствам блока сценария](img/sql_send_connection_scenario_properties_transfer.png)_
...
10. Добавьте действие «**Изменить значения переменных**» и настройте его свойства:
    ...
    - Нажмите кнопку «**Создать**» над таблицей переменных и добавьте две переменных:
        ...
        - **FilterValues** — в поле «**Значение**» выберите **атрибут** « _Код страны»_.
...
{: #step11}
    - **Подключение** — выберите ранее созданное подключение для отправки запросов в СУБД, например _«Подключение для отправки запросов в СУБД»_.
    - **Путь передачи данных** — выберите ранее созданный путь передачи данных для отправки запросов в СУБД, например _«Путь для отправки данных в СУБД»_.
...
12. За действием «**Отправить сообщение**» добавьте действие «**Повторять по количеству объектов**» и настройте его свойства:
    ...
    _![Настройка цикла по объектам в сценарии](img/sql_send_connection_scenario_cycle_settings.png)_
...
{: #step13}
    - В поле «**Целевой шаблон записи**» выберите шаблон _«Города»_, в котором будут создаваться записи с данными, полученными из внешней БД.  
        ...
        ![Настройка действия «Создать запись» в сценарии](img/sql_send_connection_create_record_settings.png)
