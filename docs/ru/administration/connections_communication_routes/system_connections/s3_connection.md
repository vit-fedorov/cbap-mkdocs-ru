...
!!! tip "Совет"
    ...
    Чтобы хранить загруженные и временные файлы отдельно от резервных копий, можно подключить дополнительное хранилище S3. Имя такого подключения должно отличаться от имени подключения к S3 для хранения резервных копий. Например:
...
## Проверка подключения к хранилищу S3 {: #s3_connection_test .pageBreakBefore .pageBreakBefore }
...
- Настройте квоты хранения данных в соответствии с бизнес-требованиями.
- Настройте минимально необходимые разрешения для корзин S3 в соответствии с их назначением.
- Регулярно проверяйте и обновляйте политики доступа к корзинам.
- Используйте разные корзины для разных типов данных (резервные копии, загруженные файлы, временные файлы).
- При необходимости включите версионирование для корзин с критически важными данными.

## Проверка работоспособности подключения к S3 {: #s3_connection_validation }
