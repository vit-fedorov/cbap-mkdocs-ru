...
## Введение {: #architecture_landscape_intro }
...
## Архитектура Системы {: #architecture_landscape_system_architecture }
...
- _Бэкенд_: компоненты-сервисы на основе .NET 8.0,  JRE 17{% if not gostech %}, Mono 6.12 (Linux), .NET Framework 4.8 (Windows){% endif %}.
- _СУБД_: фирменная патентованная СУБД **{{ productNameElasticData }}**
...
- _Модули для интеграции_: см. _«[Интеграция с внешними системами](#architecture_landscape_external_integration)»_.
...
### Безопасность и отказоустойчивость Системы {: #architecture_landscape_security_reliability .pageBreakBefore }
...
### Интеграция с внешними системами {: #architecture_landscape_external_integration }

Предусмотрены следующие механизмы интеграции с внешними системами:

- {{ gitVariants }} — контроль версий приложений, создаваемых с помощью Системы.
...
- {{ notificationServiceVariants }} — получение и отправка электронных писем.
- ESB ({{ apacheKafkaVariants }}{% if not gostech %} / RabbitMQ / MSMQ{% endif %}) — обмен сообщениями с внешними системами в распределенных и федеративных конфигурациях.

См. _«[Подключения][connections]»_.

## Ландшафт развертывания Системы {: #architecture_landscape_deployment_landscape .pageBreakBefore }
...
{% if gostech %}
### Конфигурация развертывания в контуре ЕЦП «ГосТех» {: #architecture_landscape_gostech_configuration }
...
- _{% if not gostech %}Сервер {% endif %}{{ openSearchVariants }}_ для журналирования транзакций.
- _{% if not gostech %}Обратный прокси-сервер{% endif %} {{ nginxVariants }}_ для фильтрации нежелательных запросов и ретрансляции допустимых запросов во внутреннюю сеть.
- _Сервер мониторинга {{ zabbixVariants }}_ для мониторинга доступности служб и свободного пространства на дисках.
- _{% if not gostech %}Сервер  {% endif %}{{ notificationServiceVariants }}_ (необязательно) для передачи уведомлений.
- _Сервер LDAP_ (необязательно) для централизованного управления инфраструктурой сети.
- _{% if not gostech %}Сервер {% endif %}{{ gitVariants }}_ (необязательно) для контроля версий приложений, создаваемых с помощью **{{ productName }}**.
...
### Рекомендуемый набор серверов приложений {: #architecture_landscape_application_servers .pageBreakBefore }
...
## Конфигурация подсистем {: #architecture_landscape_subsystems_configuration .pageBreakBefore }

### Конфигурация сервера распределённой СУБД {: #architecture_landscape_distributed_db_configuration }
...
### Конфигурация сервера журналирования транзакций {: #architecture_landscape_transaction_logging_configuration }
...
### Конфигурация обратного прокси-сервера {: #architecture_landscape_reverse_proxy_configuration .pageBreakBefore }
...
### Конфигурация сервера {{ zabbixVariants }} {: #architecture_landscape_monitoring_server_configuration .pageBreakBefore }
