...
Для достижения таких показателей, например, можно использовать кластерную конфигурацию с балансировкой нагрузки, резервированием компонентов и регулярным мониторингом состояния системы. См. {% if gostech %}_[документацию Platform V DataGrit](https://client.sbertech.ru/docs/public/IGN/17.0.0/IGNT/17.0.0/index.html)_{% else %}_[рекомендации по кластеризации в документации Apache Ignite (на английском языке)][apache_ignite_clustering]_{% endif %}.
...
{% else %}
- _[Документация Platform V DataGrit](https://client.sbertech.ru/docs/public/IGN/17.0.0/IGNT/17.0.0/index.html)_
{% endif %}
