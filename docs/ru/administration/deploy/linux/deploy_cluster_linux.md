...
Отказоустойчивость
    Имеем три и более узлов, из которых один является рабочим, а остальные резервные. ПО работает бесперобойно, пока сохраняет работоспособность как минимум один узел кластера. Показатели RTO/RPO удаётся свести к минимуму. Для восстановления данных достаточно резервной копии одного узла.

Распределение нагрузки
   - Имеем один узел. И нагрузку Х с временем отклика Y (равномерные небольшие запросы в 20-100 потоков). При добавлении второго узла, время отклика уменшается до (Y/2+ε). При добавлении третьего узла, время отклика уменшается до (Y/3+ε).


Высокая доступность
   - A/B обновление - Имеем три узла. При постоянной нагрузке на балансировщике производим поочередное обновление каждого из узлов до новой версии Платформы.
