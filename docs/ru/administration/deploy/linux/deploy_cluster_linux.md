...
Прежде чем приступать к развёртыванию кластера, требуется определить, какие параметры наиболее важны для бизнеса: доступность, производительность, устойчивость. В зависимости от целевой направленности экземпляра ПО следует выбрать принципы политики резервного копирования и архитектуры кластера:

- Обеспечение отказоустойчивости
    Имеется три и более узлов, из которых один является рабочим, а остальные резервные. ПО работает бесперобойно, пока сохраняют работоспособность как минимум один узел кластера и сервер данных. Показатели RTO/RPO удаётся свести к минимуму. Для восстановления данных достаточно резервной копии одного узла.
- Распределение нагрузки
   Нагрузка равномерно распределяется между узлами, что сокращает время отклика. При равномерной нагрузке время отклика можно рассчитать по формуле: `Y/N+ε`, где `Y` — время отклика при нагрузке только на один узел, `N` — количество узлов, `ε` — погрешность.
- Обеспечение высокой доступности
   При наличии трёх и более узлов в кластере и постоянной нагрузке на балансировщике возможно выполнить поочерёдное обновление каждого из узлов до новой версии ПО.
...
- сервер хранения данных на отдельной машине, к которому будут обращаться узлы кластера (для этого сервера необходимо отдельно обеспечить отказоустойчивость и высокую доступность, например, с помощью кластеризации);
