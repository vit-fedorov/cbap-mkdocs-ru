...
Прежде чем приступать к развёртыванию кластера, требуется определить, какие параметры наиболее важны для бизнеса:
...
В зависимости от критически важных целей бизнеса следует выбрать принципы архитектуры кластера:

- Обеспечение отказоустойчивости
    - Имеется три и более узлов, из которых один является рабочим, а остальные резервные. ПО работает бесперебойно, пока сохраняют работоспособность как минимум один узел кластера и сервер данных. Показатели RTO/RPO удаётся свести к минимуму. Для восстановления данных достаточно резервной копии одного узла.
