...
### Топология и консистентность

Настройте базовую топологию кластера, чтобы минимизировать риски при добавлении новых узлов и восстановлении после сбоев. См. _«[Настройка базовой топологии. Документация Apache Ignite][apache_ignite_partition_baseline_topology]»_.

Чтобы повысить надёжности в кластере с большим количеством узлов, можно использовать Zookeeper Discovery SPI для управления обнаружением узлов и уменьшения рисков, связанных с сетевой фрагментацией. См. _«[Настройка ZooKeeper Discovery. Документация Apache Ignite][apache_ignite_partition_zookeeper_discovery]»_.
...
Для поддержания высокой доступности и своевременного реагирования на инциденты:

- настройте интеграцию с инструментами мониторинга, такими как Prometheus или Grafana, для сбора метрик по работе кластера;
- включите уведомления для критичных событий (например, падение узла или превышение времени отклика);
- используйте встроенные возможности Apache Ignite для мониторинга производительности и нагрузки на узлы.
