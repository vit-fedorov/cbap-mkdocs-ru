...
{: #install.sh}
    ...
    ``` sh
    sh install.sh -p [-k] [-i=<instanceName>] [-e] [-d=demo]
    ```
...
7. Установите следующую директиву:
...
3. Разверните экземпляр ПО:
    ...
    - `-v=<versionNumber>` — номер версии ПО вида `X.X.XXXX.X` (например: 4.7.2222.0, **обязательный** ключ). Версия должна быть установлена, см. _«[Установка {{ productName }}](#deploy_guide_linux_install_sw)»_.
...
3. Просмотрите список имеющихся экземпляров ПО:

    ``` sh
    cat /usr/share/comindware/configs/instance/* | grep -E '(configPath:)'
    ```

4. Просмотрите список используемых портов:
...
6. Создайте новый экземпляр ПО согласно приведённым выше [инструкциям](#создание-экземпляра-по), указав для него уникальные имя и порт.
7. Откройте для редактирования три службы каждого из установленных экземпляров ПО (`<instanceName>`):
...
8. Если используются локальные службы Kafka и Elasticsearch, откройте их для редактирования:
...
9. В каждом файле службы установите следующие директивы:
