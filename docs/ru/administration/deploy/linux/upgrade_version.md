...
{: #dataUpgrade }
    ...
    - Удостоверьтесь, что появился журнал обновления `/var/log/comindware/<instanceName>/Log/UpgradeOntology.log`.
    - Удостоверьтесь, что в журнале обновления последняя запись имеет статус `successfully`.
    - Удостоверьтесь, что в журнале отсутствуют ошибки обновления. Найдите их по ключевому слову `error`.
