...
1. Создайте резервную копию экземпляра ПО:
...
3. Распакуйте резервную копию и удалите из неё следующие директории кэшей:
...
4. Очистите директорию с базой данных экземпляра ПО:

    ``` sh
    rm -rf /var/lib/comindware/<instanceName>/Database/*
    ```

5.  Скопируйте очищенную резервную копию в директорию с базой данных `/var/lib/comindware/<instanceName>/Database/`.
6.  Назначьте владельца директории с базой данных:
...
8.  Откройте сайт экземпляра ПО в браузере, дождитесь его инициализации и выполните вход.
...
1. Подготовьте инфраструктуру к переключению на новый IP-адрес:
    ...
    - проверьте доступность нового IP-адреса.
...
1. При обнаружении ошибок в журналах обновления:
    ...
    - верните DNS-записи и IP-адреса на старый сервер;
