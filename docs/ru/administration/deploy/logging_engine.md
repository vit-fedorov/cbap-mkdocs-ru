...
#### Правила формирования файлов журналов {: #logging_engine_rules }

Для каждого журнала ежедневно формируется один файл.

По умолчанию ПО сохраняет в директории `Logs` все файлы журналов за последние 50 дней. Файлы журналов старше 50 дней удяляются.

Лимит файлов журналов каждого типа задаётся директивой `maxArchiveFiles` в файле `logs.config`. См. _«[Описание подсистемы журналирования](#logging_engine_description)»_.
