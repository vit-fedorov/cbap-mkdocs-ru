...
## Описание подсистемы журналирования {: #logging_engine_description }
...
## Файловые журналы {: #logging_engine_file_logs .pageBreakBefore }

ПО поддерживает следующие файловые журналы:

- [Адаптеры](#logging_engine_adapter_logs)
- [Аудит](#logging_engine_audit_log)
- [Интеграция](#logging_engine_integration_log)
- [Интеграция — сырые данные](#logging_engine_integration_raw_log)
- [Исправность ПО](#logging_engine_heartbeat_log)
- [Обновление](#logging_engine_update_log)
- [Ошибки](#logging_engine_error_log)
- [Процессы](#logging_engine_process_log)
- [Системный журнал](#logging_engine_system_log)
- [Резервное копирование](#logging_engine_backup_log)
- Трансфер — в составе журнала [ошибок](#logging_engine_error_log)
...
#### Правила формирования файлов журналов {: #logging_engine_rules .pageBreakBefore }
...
Лимит файлов журналов каждого типа задаётся директивой `archiveFilesCount` в файле `logs.config`. См. _«[Описание подсистемы журналирования](#logging_engine_description)»_.

#### Типы событий в журналах {: #logging_engine_event_types .pageBreakBefore }
...
#### Журнал интеграции {: #logging_engine_integration_log .pageBreakBefore }
...
#### Журнал интеграции с сырыми данными {: #logging_engine_integration_raw_log .pageBreakBefore }
...
#### Журнал исправности {: #logging_engine_heartbeat_log .pageBreakBefore }
...
#### Журнал обновления {: #logging_engine_update_log .pageBreakBefore }
...
#### Журнал ошибок {: #logging_engine_error_log .pageBreakBefore }
...
#### Журнал процессов {: #logging_engine_process_log .pageBreakBefore }
...
##### Рекомендации по чтению журнала процессов {: #logging_engine_process_log_recommendations .pageBreakBefore }
...
#### Журнал резервного копирования {: #logging_engine_backup_log .pageBreakBefore }
...
#### Журнал транзакций {: #logging_engine_performance_log .pageBreakBefore }

Журнал транзакций содержит сведения о ходе выполнения транзакций.

Стандартное имя файла журнала транзакций: `performance_ГГГГ-ММ-ДД.log`

Журнал транзакций содержит следующие сведения:

- идентификатор запроса для корреляции;
- время события;
- длительность выполнения;
- идентификатор записи;
- идентификатор корневого запроса цепочки;
- идентификатор родителя в иерархии вызовов;
- количество повторений операции;
- имя кластера в многоузловой конфигурации;
- имя узла в кластере;
- сведения о действиях, исполненных в ходе транзакции:
    - тип записи;
    - компонент-источник;
    - URL вызова, имя метода, действие;
    - идентификатор сессии пользователя;
    - имя аккаунта, инициировавшего событие;
    - идентификатор запроса для корреляции.

_![Пример журнала транзакций](img/logging_engine_performance_log.png)_

#### Системный журнал {: #logging_engine_system_log .pageBreakBefore }
...
## Перенаправление сообщений из файлов журналов в журнал событий Windows {: #logging_engine_windows_eventlog .pageBreakBefore }
