...
15. Проверьте, что ПО установлено:
...
1. Установите экземпляр ПО:
    ...
    - `port <portNumber>` — порт для экземпляра ПО (необязательный ключ). По умолчанию: 80.
...
2. Удалите экземпляр ПО:
    ...
    - `clear` — удалить все файлы, папки и службы экземпляра ПО.
...
3. Удалите все экземпляры с версией ПО, которую требуется удалить, или обновите их до другой версии. Удалить версию ПО, которая используется в каких-либо экземплярах, не удастся.
4. Удалите версию ПО:
