...
Адаптер состоит из перечисленных ниже компонентов. 
...
- ДОБАВИТЬ ОПИСАНИЕ ПУТИ ПЕРЕДАЧИ ДАННЫХ И ПОДКЛЮЧЕНИЯ
...
- Инфраструктурные библиотеки:
    ...
    - Comindware.MessageBroker.Api — библиотека ...
    - Comindware.Platform.FormDefinition — библиотека для формирования диалоговых окон.
- Библиотеки для получения сообщений:
    - Comindware.Adapter.IncomingService
- Библиотеки для отправки
    - Comindware.Adapter.OutgoingService
...
### Структура классов и методов адаптера для получения сообщений
...
- Папка `Behaviour`
  - Класс `Comindware.Adapter.IncomingPerformer`
    ...
    | `SendRawResponse` | Вызов функции `IIncomingConnectionService.OnReplyReceive(object message)` для успешного ответа или `IIncomingConnectionService.OnErrorReceive(object message)` для отправки сообщения с ошибкой |
  - Класс `Service`
    | Метод | Описание |
    |---|---|
...
- Папка `Signature`
  ...
  - Класс `{communicationRouteName}IncomingMessage`
    | Метод | Описание |
    |---|---|
    | `{communicationRouteName}IncomingMessage` | Модель сообщений, унаследованная от Comindware.Adapter.Message |
  - Класс `ErrorMessage`
    | Метод | Описание |
    |---|---|
...
### Структура классов и методов адаптера для отправки сообщений
...
- Папка `Behaviour`
  - Класс `Comindware.Adapter.OutgoingPerformer`
    ...
    | `RecognizeRawResponse` | Обработка ответного сообщения внешнего сервиса, возвращает объект ProcessingResult с ProcessingResultType.MessageRecognized и ResponseMessage или ProcessingResult с ProcessingResultType.ErrorMessageRecognized и ErrorMessage |
  - Класс `Service`
    | Метод | Описание |
    |---|---|
    | `CreatePerformer` | Инициализация перформера |
    | `PerformTest` | Проверка соединения с внешним сервисом, вызывается нажатием соответствующей кнопки в окне Подключение и логгером состояния Платформы с определённой периодичностью |
...
- Папка `Signature`
  ...
  - Класс `{communicationRouteName}Request`
    | Метод | Описание |
    |---|---|
    | `{communicationRouteName}Request` | Модель сообщений, унаследованная от Comindware.Adapter.Message |
  - Класс `ErrorMessage`
    | Метод | Описание |
    |---|---|
    | `ErrorMessage` | Модель сообщения об ошибке, унаследованная от Comindware.Adapter.Message |
