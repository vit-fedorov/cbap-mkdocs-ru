...
!!! warning "Внимание!"
    ...
    Основные параметры глобальной конфигурации настраиваются в [конфигурационном файле экземпляра ПО][configuration_files_linux]. Здесь представлены инструкции по настройке параметров, доступных непосредственно на сервере экземпляра **{{ productName }}**.
