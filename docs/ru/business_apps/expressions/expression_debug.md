...
!!! note "Примечание"
    ...
    Полный редактор выражений позволяет сохранять формулы, выражения N3 и скрипты только без ошибок.
...
6. Введите в поле _AmountText_ значение `10.500`, то в поле атрибута отобразится `-`.
...
!!! question "try-catch"
    ...
    - Поместите в блок `try` код, исключения в котором требуется обрабатывать.
...
!!! question "Действие «Проверить результат выражения»"
    ...
    Это действие проверяет выполнение заданного условия в контекстном шаблоне. Если условие не будет выполнено, в цепочке событий вывводится сообщение об ошибке с заданным текстом.
