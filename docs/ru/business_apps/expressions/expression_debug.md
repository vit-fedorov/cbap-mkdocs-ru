...
2. Если в скрипте, формуле или выражении содержатся синтаксические ошибки и ошибки в названии переменных, редактор сообщит об этом.
3. Исправьте ошибки и проверьте скрипт, выражение или формулу ещё раз.

!!! note "Примечание"

    Полный редактор выражений позволяет сохранять скрипты, выражения и формулы только без ошибок синтаксиса и ошибок в названии переменных.
...
## Отладка формул и выражений с помощью функций
...
`"Ошибка"` — сообщение, которое будет выводиться при неправильном выполнении первого аргумента `<вычисляемое выражение>`. Также вместо сообщения об ошибке может быть задано значение по умолчанию.
...
Для поиска ошибок в C#-скриптах можно воспользоваться блоком **try-catch** для перехвата ошибок.
...
1. После действия или группы действий добавьте блок «**Проверить результат выражения**».
...
4. Запустите экземпляр процесса и перейдите к его диаграмме.
