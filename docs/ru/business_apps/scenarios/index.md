...
!!! question "Определения"
	...
	**Сценарий** — это алгоритм валидации и обработки данных. Сценарий содержит цепочку из [действий][scenario_actions], которая будет запущена при срабатывании определённого [события][scenario_event].
...
Сценарии служат отличным дополнением к бизнес-процессам, однако не являются их альтернативой. Сценарии могут происходить внутри процессов, так и независимо от них.

Сценарии позволяют решать различные задачи, например:

- Создание, дублирование и изменение записей, см. _[«Копирование записи вместе с прикреплённым документом. Настройка сценария»][attribute_document_clone_scenario]_.
- Получение и отправка электронной почты, см.:
...
- Обмен данными с внешними системами, см.:
...
- Получение и отправка HTTP-запросов, см.:
...
- Внесение изменений в данные пользователем без учёта роли и разрешений, см. _[«Настройка свойств сценария»](#настройка-свойств-сценария)_.
- Подготовка документов по образцу и использование электронной подписи, см.:
...
- Принудительная остановка бизнес-процессов и запрет на внесение изменений в записи, см. [действие сценария _«Проверить результат выражения»_][проверить-результат-выражения].
