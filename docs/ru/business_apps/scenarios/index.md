...
**Сценарий** представляет собой алгоритм автоматизации операций, обработки и передачи данных, который запускается при наступлении заданного [**события**][scenario_event] и выполняет настроенную последовательность [**действий**][scenario_actions]. 

Помимо обработки данных внутри **{{ productName }}** сценарии позволяют обмениваться данными с внешними системами.

Сценарии дополняют бизнес-процессы, однако не являются их альтернативой, так как не позволяют реализовать логику BPMN. Сценарии могут использоваться независимо в тех случаях, когда нет потребности в создании бизнес-процесса.
...
Сценарии являются инструментом автоматизации в приложениях. Они дополняют бизнес-процессы, однако не являются их альтернативой, так как не позволяют реализовать логику BPMN.

<!-- Рассказать в каких случаях следует применять сценарии, а в каких — процессы. Этот запрос есть в изначальной заявке -->

<!-- Известны ли каки-либо ограничения на работу сценариев? Количество? Параллелизм? Иное? -->

Сценарии имеют ряд особенностей:

- Сценарий выполняется один раз при каждом наступлении заданного события.
- Сценарий выполняется в контексте определённого шаблона записи. Однако некоторые действия сценария, например «**Сменить контекст**» или «**Создать запись**», позволяют переключаться между шаблонами и манипулировать данными в них. _См. подробнее [«Событие и действия сценария. Определения, типы, свойства, настройка»][scenario_elements]_.
...
<!--Возможно, ремарку про бесконечные циклы следует перенести в статью по отладке-->
...
<!--Возможно, всё, что касается отладки сценариев, можно перенести в статью про сценарии? Хотя, статья про отладку может потерять целостность и размазаться-->

## Примеры использования сценариев

Сценарии позволяют решать различные задачи, например:

<!--Какой здесь порядок следования примеров? Алфавитный? По приоритету примеров? Иной? -->
<!--Возможно, стоит отсортировать по алфавиту?-->
