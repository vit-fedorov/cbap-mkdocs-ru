...
**Сценарий** представляет собой алгоритм обработки данных:

Сценарий запускается при наступлении заданного [события][scenario_elements] и выполняет настроенную последовательность [действий][scenario_elements].

Для составления сценариев предусмотрен [визуальный конструктор](scenario_configure.md).

Для запуска сценариев предусмотрены следующие события:
...
В сценариях предусмотрены следующие типы действий:
...
- При запуске для нескольких записей (например, выбранных в таблице) сценарий выполняется отдельно каждой записи. То есть в контексте сценария видна только одна запись, по которой он запущен.
