...
С помощью этого действия можно осуществлять отладку сценариев и, например, устанавливать запрет на сохранение записей. См. _[«Отладка формул, выражений N3, сценариев и C#-скриптов»][expression_debug]_ и [_«Сценарий. Проверка и изменение данных с помощью действия «Проверить результат скрипта»_][scenario_verify_data_by_script].
