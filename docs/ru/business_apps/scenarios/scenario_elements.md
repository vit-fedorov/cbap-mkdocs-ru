...
kbTitle: Событие и действия сценария. Определения, типы, свойства, настройка
tags:
  - сценарии
  - сценарий
  - событие
...
## Введение {: #scenario_elements_intro }
...
### Общие свойства событий {: #scenario_events_common_properties }
...
### Свойства событий по типам {: #scenario_events_properties_by_type }
...
Сценарий будет запускаться перед выполнением элемента диаграммы **[контекстного шаблона процесса](#scenario_events_common_properties)**.
...
!!! question "Определения"
    ...
    **Действие сценария** — это элемент, который выполняется после наступления **события сценария**.
    ...
    У действий можно настроить [общие](#scenario_actions_common_properties) и [специфические для каждого типа](#scenario_actions_properties_by_type) свойства.
...
### Свойства действий по типам {: #scenario_actions_properties_by_type .pageBreakBefore }
...
#### Дублировать запись {: #scenario_actions_clone_record .pageBreakBefore }
...
#### Сменить контекст {: #scenario_actions_change_context .pageBreakBefore }
...
#### Изменить значения атрибутов {: #scenario_actions_change_attribute_values .pageBreakBefore }
...
#### Создать документ по шаблону {: #scenario_actions_create_document_from_template .pageBreakBefore }
...
#### Прикрепить документ к атрибуту {: #scenario_actions_attach_document_to_attribute .pageBreakBefore }

Это действие помещает в атрибуты типа «**Документ**» ссылки на файлы, сформированные с помощью действий «[**Создать документ по шаблону**](#scenario_actions_create_document_from_template)».
...
#### Выполнить по условиям {: #scenario_actions_execute_by_conditions .pageBreakBefore }
...
#### Повторять по числовому счётчику {: #scenario_actions_repeat_by_counter .pageBreakBefore }
...
#### Повторять по количеству объектов {: #scenario_actions_repeat_by_object_count .pageBreakBefore }
...
#### Запустить процесс {: #scenario_actions_start_process .pageBreakBefore }

Это действие создаёт экземпляр выбранного шаблона процесса.

**Настройка действия**

- **Шаблон процесса** — шаблон процесса, в котором будет создан новый экземпляр по текущей записи [**контекстного шаблона**](#scenario_events_common_properties) (указан на вкладке «**Дополнительно**»).
...
#### Изменить значения переменных {: #scenario_actions_change_variable_values .pageBreakBefore }
...
#### Изменить значения скриптом {: #scenario_actions_modify_value_csharp .pageBreakBefore }
...
- _[Использование переменных приложения в C#-скриптах][variables_csharp]_
...
- [Использование переменных приложения в C#-скриптах][variables_csharp]
