...
Сценарии состоят из одного события, которое запускает его выполнение, и последовательности действий. После запуска события выполняются действия сценария по порядку.

!!! question "Событие сценария"

    **Событие** — это элемент, наступление которого запускает сценарий.

    При создании нового сценария автоматически добавляется событие «**Нажата кнопка**», кроме создания сценария на входе или выходе токена.
...
##### Пример сценария на ограничение редактирования записей
