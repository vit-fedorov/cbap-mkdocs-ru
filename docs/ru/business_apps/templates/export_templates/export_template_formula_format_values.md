...
При указании количества знаков после запятой в свойствах атрибут типа «**Число**» экспортируется с соответствующим форматированием ячейки в Excel. Если в свойстве атрибута «**Количество знаков после запятой**» выбрать пункт «**Не преобразовывать**», то при экспорте в Excel данные будут сохранены в ячейку типа «Общий».

Также в шаблоне экспорта можно задать ячейке числовой формат с указанием количества знаков после запятой, по которому будут преобразованы данные при экспорте:
...
- заменитель `{durationAttributeSystemName:dd' д 'hh' ч'}` будет экспортирован в формате `1 д 3 ч`.
