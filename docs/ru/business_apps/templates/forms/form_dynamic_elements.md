...
Если для поля атрибута типа «**Запись**» выбрано **представление** «**Карта**», то в поле будет отображаться карта с географическими метками, сформированными по записям связанного шаблона. Конечный пользователь сможет искать адреса, добавлять метки на карту и убирать их с карты:

- при добавлении метки на карту будет создаваться запись в связанном шаблоне с данными метки в указанных атрибутах;
- при удалении метки с карты, относящаяся к ней запись не удаляется из шаблона;
- при добавлении записи с допустимыми широтой и долготой метки в связанный шаблон метка появится на карте.

1. Создайте подключение к картам с помощью ключа API. См. статью «».
2. Создайте шаблон записи для хранения картографических данных со следующими атрибутами:
...
!!! note "Примечание"
    ...
    Для использования представления «**Карта**» необходимо предварительно настроить приложение:

    - На странице «**Администрирование**» — «**Подключения**» настройте [подключение к картографической службе][maps_connection].
    - В шаблоне, связанном с атрибутом типа «**Запись**», создайте атрибуты для хранения картографических данных:
        - атрибуты типа «**Текст**»: 
            - _Страна_
            - _Населённый пункт_
            - _Улица_
...
- **Высота** — задайте высоту карты на форме.
...
- **Атрибуты адреса** — выберите из связанного шаблона атрибуты типа «**Число**» (из связанного шаблона) для хранения географических координат меток на карте:
