...
4. Поместите атрибуты _«Наименование пакета документов»_ и _«Файлы документов»_ на основную форму шаблона _«Пакеты документов»_.
...
7. После события «**Изменить значения переменных**» создайте и настройте действие «**Выполнить по условиям**» следующим образом:
    ...
    !!! warning "Бизнес-логика"
        ...
        Это действие будет инициировать вложенные в него действия, если в текущей записи _«Пакеты документов»_ к атрибуту _«Файлы документов» (_с системным именем `AttachedFiles`) прикреплён хотя бы один файл.
...
11.  Внутри действия «**Повторять по количеству объектов**» создайте и настройте действие «**Изменить значение атрибутов**» следующим образом:
    ...
    !!! warning "Бизнес-логика"
        ...
        Это действие будет прикреплять к атрибуту _«Файлы документов»_ в новой записи дубликат файла, прикреплённого к  атрибуту _«Файлы документов»_из исходной записи.
...
8. Откройте новую запись и убедитесь, что содержимое полей _«Наименование пакета документов»_ и _«Файлы документов»_ совпадает с содержимым исходной записи_._
