---
title: Скачивание архива с файлами из таблицы
tags:
    - C#
    - C#-скрипт
    - скрипт
...
# Скачивание архива с файлами из таблицы с помощью C#-скрипта {: #download_archive_csharp}
...
Здесь представлен пример настройки кнопки, скачивающей файлы из выбранных записей в таблице. На форме эта кнопка будет скачивать файлы, прикреплённые к атрибуту типа «**Документ**» в текущей записи.
...
2. Создайте следующие **атрибуты**:
    ...
    | -------------- | ------------ | ----------------------------------------------------------------------------------------------------------- |
    | *Наименование* | **Текст**    |                                                                                                             |
    | *Вложения*     | **Документ** | **Хранить несколько значений:** флажок установлен<br>**Использовать для поиска записей:** флажок установлен |

    !!! warning "Внимание"

        Для корректного выполнения C#-скрипта требуется обязательно установить флажок «**Использовать для поиска записей**». В ином случае, скрипт не сработает.

4. Создайте [**кнопку**][buttons] _«Скачать вложения»_ со следующими свойствами:
...
7. Поместите атрибуты _«Наименование»_ и _«Вложения»_ в таблицу _«Все записи»_ и на форму шаблона _«Реестр документов»_.
8. Поместите кнопку _«Скачать вложения»_ в **таблицу** _«Все записи»_ шаблона _«Реестр документов»_.
9. В **шаблоне записи** _«Заявка»_ создайте **атрибут**:

    | Название   | Тип данных | Свойства                                                                                       |
    | ---------- | ---------- | ---------------------------------------------------------------------------------------------- |
    | *Документы* | **Запись** | **Связанный шаблон:** _Реестр документов_<br>**Хранить несколько значений:** флажок установлен |

10. Поместите на форму шаблона _«Заявка»_ **атрибут** _«Документы»_ и настройте его **представление** в виде **таблицы**.
11. Поместите в таблицу _«Документы»_ на форме **атрибуты** _«Наименование»_ и _«Вложения»_ шаблона записи _«Реестр документов»_.
12. В область кнопок таблицы _«Документы»_ поместите кнопки «**Создать**» и _«Скачать вложения»_.
