---
title: Локальные переменные и C#-скрипт. Копирование данных между шаблонами
tags:
    - C#
    - скрипт
    - C#-скрипт
...
# Локальные переменные и C#-скрипт. Копирование данных между шаблонами
...
Здесь представлена инструкция по настройке копирования данных из справочника в шаблон записи с использованием C#-скрипта и локальной переменной по нажатию кнопки.
...
6. Вынесите локальную переменную _«Конкурс»_ на форму и сохраните.
...
9. Сохраните кнопку.
10. Вынесите кнопку _«Добавить позиции в конкурс»_ на форму таблицы записей шаблона _«Номенклатура продукции»_.

### Настройка кнопки с использованием сценария

1. Добавьте к шаблону _«Позиции в конкурсе»_ атрибут типа **запись** _«Исходная позиция»_, связанный с шаблоном _«Номенклатура продукции»_.
2. Создайте для шаблона записи «Номенклатура продукции» кнопку _«Добавить позиции в конкурс»_ и добавьте следующую локальную переменную:
...
5. Нажмите гиперссылку «**Настроить диалоговое окно**».
6. Вынесите локальную переменную _«Конкурс»_ на форму и сохраните.
7. Вынесите кнопку _«Добавить позиции в конкурс»_ на форму таблицы записей шаблона _«Номенклатура продукции»_.
8. Создайте новый сценарий _«Добавить позиции в конкурс»_.
9. В начальном событии «**Нажата кнопка**» выберите контекстный шаблон _«Номенклатура продукции»_ и кнопку _«Добавить позиции в конкурс»_.
10. Добавьте новое событие «**Создать запись**» с целевым шаблоном _«Позиции в конкурсе»_.
11. Внутри события «**Создать запись**» добавьте событие «**Изменить значения атрибутов**».
12. Добавьте следующие атрибуты для изменения:

    | Атрибут           | Операция со значениями        | Значение |
    | ------------------ | ---------- | ------------------ |
    | _Конкурс_         | **Заменить**  | <p>**Формула:** `$$dialogVariables->konkurs`</p><p>**где:**</p><p>`dialogVariables` —  локальные переменные кнопки, которая вызвала сценарий по событию «Нажатие кнопки».</p><p>`konkurs` — системное имя локальной переменной _«Конкурс»_.</p> |
    | _Исходная позиция_ | **Заменить** | <p>**Формула:** `$$origin`</p><p>**где:**</p><p><p>`origin` —  ID записи, для которой была нажата кнопка.</p> |
    | _Наименование_ | **Заменить** | <p>**Атрибут:** _Исходная позиция_->_Наименование_</p> |
    | _Единица измерения_ | **Заменить** | <p>**Атрибут:** _Исходная позиция_->_Единица измерения_</p> |
    | _Цена_ | **Заменить** | <p>**Атрибут:** _Исходная позиция_->_Цена_</p> |
