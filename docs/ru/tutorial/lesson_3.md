...
3. Добавим на диаграмму дорожки.
    ...
    !!! question "Дорожка"
        ...
        **Дорожка** в BPMN представляет участников процесса. Дорожки можно настроить произвольным образом по усмотрению проектировщика процесса. Обычно они представляют клиента, отдел, должность, роль или команду. Например, в нашем процессе _Заказчик_ — это роль (_Заказчик_ может быть из любого отдела), _Гараж_ — отдел, а _Секретарь_ — должность.
...
8. Удалите конечное событие. Для этого выберите его и в меню элемента нажмите кнопку «**Удалить**» <i class="fa-light fa-trash-can"></im>.
...
15. К развилке _«Одобрена?»_ присоедините потоками управления две новые пользовательских задачи:
...
21. Присвойте конечному событию название _«Отказ»_.
    ...
    !!! question "События"
        ...
        **События** в BPMN можно рассматривать как этапы процесса — старт, промежуточные вехи, варианты завершения. У процесса может быть только одно начальное событие, а конечных событий несколько для различных результатов выполнения процесса.
...
29. Добавьте конечное событие и назовите его _«Успех»._
...
7. Перетащите в область _«Решение»_ атрибут _«Одобрено»_ из панели элементов.
...
Настроим форму задачи, которую будет заполнять Сотрудник гаража.

1. Выберите задачу _«Принять заявку»_ на диаграмме процесса и в меню элемента нажмите кнопку «**Форма**» <i class="fa-light fa-newspaper"></i>. Отобразится конструктор формы.
...
4. Нажмите кнопку «**Добавить атрибут**» <i class="fa-light fa-plus"></i> рядом с пунктом _«Заявки на автомобили»_ в панели элементов, чтобы добавить атрибут в этот шаблон записи.
...
7. Перетащите на макет формы новую **область** и переименуйте её в _«Принятие заявки»_ с помощью панели свойств.
...
10. Сохраните форму и вернитесь к диаграмме процесса с помощью кнопки «**Назад**» браузера.
...
Перейдем к настройке потоков управления после развилок. На диаграмме есть две развилки — _«Одобрена?»_ и _«Есть машина?»_.

### Развилка «Одобрена?»

!!! warning "Бизнес-логика"

    Поток, по которому процесс пойдёт после развилки _«Одобрена?»_, задаёт логический атрибут _«Одобрено»_, значение которого устанавливает _Секретарь_ при рассмотрении заявки.

    Для управления ходом процесса определим условие для потока к задаче _«Принять заявку»_ на основе значения атрибута _«Одобрено»_.
...
4. Составим выражение для условия, при выполнении которого процесс пойдет дальше. Для этого нам потребуется системное имя атрибута _«Одобрено»._
...
7. При составлении выражения необходимо обратиться системному имени атрибута.
8. Введите символ `$` и в отобразившемся раскрывающемся списке дважды нажмите системное имя атрибута `Одобрено`.
    ...
    !!! question "Ссылки на атрибуты в формулах"
        ...
        То есть формула `$Одобрено` возвращает значение атрибута _«Одобрено»_ — `true` или `false` (ложь или истина).
...
!!! tip "Совет"
    ...
    Для упрощения на этапе отладки процесса укажите исполнителем этих задач одного и того же пользователя, например свой аккаунт.
...
1. Нажмите кнопку «**Проверить**» в конструкторе диаграммы процесса.
    ...
    -Если обнаружены какие-либо ошибки, элементы с ошибками будут выделены красным цветом. Щелкните восклицательный знак рядом с выделенным элементом, чтобы просмотреть сообщения об ошибках.
...
13. Установите флажок _«Одобрено»_, то есть одобрите заявку в роли _Секретаря_.
...
21. Откройте задачу и снимите флажок _«Одобрено»_, то есть отклоните заявку в роли _Секретаря_.
22. Завершите задачу.
