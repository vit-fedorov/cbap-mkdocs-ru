...
6. Внутри действия «**Сменить контекст**» нажмите кнопку «**Добавить действие**» <i class="fa-light fa-circle-plus"></i>  и в раскрывающемся меню выберите пункт «**Изменить значения атрибутов**».
...
5. Введите формулу: `FORMAT("Рассмотреть заявку ({0} — {1})",LIST($Маршрут,$_creator->fullName))`.

    Здесь:

    - `$Маршрут` — системное имя атрибута _«Маршрут»._
    - `$_creator->fullName` — имя пользователя, создавшего заявку.
