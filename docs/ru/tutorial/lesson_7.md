...
1. Перейдите на вкладку «**Формы**» шаблона записи *«Заявки на автомобиль».*
...
4. В конструкторе формы разверните элемент *«Заявки на автомобиль»* на панели элементов.
...
15. В отобразившемся раскрывающемся списке дважды нажмите пункт `id`. в формулу будет вставлено системное имя `id` атрибута *«ID»* текущего шаблона *«Заявки на автомобиль».*

    _![Вставка в выражение системного имени атрибута «ID» шаблона «Заявки на автомобиль»](https://kb.comindware.ru/assets/img_6475c34222af8.png)_
...
1. Перейдите на вкладку «**Атрибуты**» шаблона записи *«Заявки на автомобиль».*
...
15. Ведите формулу: `FORMAT("ЗA-{0}",LIST(COUNT(from b in db->Заявканаавтомобиль select b->id)))`. Нажмите кнопку с зеленым флажком.
    ...
    - `Заявканаавтомобиль` — это системное имя шаблона записи *«Заявки на автомобиль».*
...
1. Перейдите на вкладку «**Формы**» шаблона записи *«Заявки на автомобиль».*
2. Откройте конструктор формы *«Заявки на автомобиль — Основная форма»* и добавьте на нее поле «Номер заявки».
...
10. Вернитесь к диаграмме модели данных и выберите шаблон *«Заявки на автомобиль».*
...
Настроим отображение реестра (шаблона записи) *«Заявки на автомобиль»*…

1. Перейдите на вкладку «**Таблицы**» шаблона «Заявки на автомобиль» и добавьте в таблицу «**Все записи**» столбцы *«Номер заявки»* и *«Статус»*.
2. Сохраните таблицу.
...
5. Введите формулу: `FORMAT("Согласовать заявку ({0} — {1})",LIST($Маршрут,$_creatorRef->fullName))`.
...
7. Аналогичным образом введите формулы для остальных задач процесса, указав в выражении соответствующие имена задач:
    ...
    - `FORMAT("Вам отказано ({0} — {1})",LIST($Маршрут,$_creatorRef->fullName))`
    - `FORMAT("Выполнить рейс ({0} — {1})",LIST($Маршрут,$_creatorRef->fullName))`
    - `FORMAT("Машина выделена ({0} — {1})",LIST($Маршрут,$_creatorRef->fullName))`
