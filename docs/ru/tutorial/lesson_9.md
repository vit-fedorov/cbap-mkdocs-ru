...
- `Затраты` — системное имя атрибута типа «**Запись***» шаблона *«Заявки на автомобиль»*, связанного с шаблоном *«Затраты»*.*
- `Сумма` и `Типзатрат` — системные имена атрибутов шаблона *«Затраты»*.
- `Типзатрат` — атрибут типа «**Запись**» в шаблоне *«Затраты»*, связанный с шаблоном *«Тип затрат»*. С помощью символа точки мы обращаемся к атрибуту `Название` шаблона *«Тип затрат»*.

Кроме того, мы используем системные имена следующих атрибутов шаблона *«Заявки на автомобиль»*:
...
1. Перейдите на вкладку «**Шаблоны экспорта**» шаблона записи *«Заявки на автомобиль»*.
2. Нажмите кнопку «**Создать**».

    _![Создание шаблона экспорта для шаблона записи «<i>Заявки на автомобиль</i>»](https://kb.comindware.ru/assets/img_63175c2c30b3d.png)_
...
1. Перейдите на вкладку «**Кнопки***» шаблона *«Заявки на автомобиль»*.*
...
1. Перейдите на вкладку «**Формы**» шаблона *«Заявки на автомобиль»*.
2. Нажмите кнопку «**Создать**».
...
3. Разверните в панели элементов шаблон *«Заявки на автомобиль»* и перетащите форму *«Выгрузка отчета»* под область *«Выполнить рейс»*.
4. Сохраните форму.
...
3. Раскройте в панели ресурсов шаблон *«Заявки на автомобиль»* и перетащите из него на таблицу разрешений кнопку *«Авансовый отчет»*.
...
!!! note "Примечание"
    ...
    Если вы проходите этот урок после прохождения [Урока 8 *«Аккаунты»*][lesson_8], то для тестирования работы приложения вы можете выполнять пользовательские задачи одним из трех способов:
