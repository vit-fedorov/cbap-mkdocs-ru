...
**Comindware Business Application Platform** может обращаться к API внешних систем через [подключения](https://kb.comindware.ru/category.php?id=437) и [пути передачи данных](https://kb.comindware.ru/category.php?id=515), а также может обрабатывать запросы из внешних систем через интерфейсы **REST API** трех типов:
...
- **Comindware Business Application Platform** возвращает ответ формате JSON или XML.
...
- При вызове методов API в интерфейсе [Swagger](#mcetoc_1h7csahj80) на странице `https://your-host/docs` уже выполнен вход в **Comindware Business Application Platform**, и дополнительная аутентификация не требуется.
...
- Запрос отправляет скрипт на C# (вызываемый [кнопкой](https://kb.comindware.ru/article.php?id=2276#mcetoc_1gk3dof774) или [задачей выполнения сценария](https://kb.comindware.ru/article.php?id=22388) в процессе). Скрипт устанавливает подключение к внешней системе, формирует запрос, отправляет его и получает ответ.
...
- Запрос формирует и отправляет [промежуточное][send_message_end_event] отправки сообщения в процессе.

Примечание

Для отправки запросов с помощью сценариев и событий процесса требуется настроить [подключение][communication_routes]. 
...
_![Страница Swagger UI с разделами API Comindware Business Application Platform](https://kb.comindware.ru/assets/img_661d06c21de49.png)_
...
- В ответ на запрос **Comindware Business Application Platform**вернёт ID созданного шаблона записи (например, `"oa.1"`).
