...
3. **Для компиляции, настройки и использования адаптера в {{ productName }}**
   ...
   - Доступ к средствам мониторинга адаптеров для диагностики возможных проблем. См. *«[Мониторинг работы адаптера](#adapters_monitor)»*.
...
9. Отслеживайте работу адаптера, см. *«[Мониторинг работы адаптера](#adapters_monitor)»*.
...
_![Пример содержимого архива с исходным кодом адаптера](/platform/v5.0/administration/infrastructure/img/adapter_archive_content.png)_
...
- `{CommunicationRoutePath}` — URI эндпоинта адаптера, например: `MyIncomingAdapter/GetRequests`.
  ...
  См. *«[Настройка сервера {{ productName }} для работы адаптера](#adapters_server_configure)»*.
...
## Настройка сервера {{ productName }} для работы адаптера
...
2. Отобразится список адаптеров.
   ...
   ![Список адаптеров](/platform/v5.0/administration/infrastructure/img/adapter_list.png)

   Список адаптеров
3. Нажмите кнопку «**Создать**» или дважды нажмите строку имеющегося адаптера.
4. Отобразится страница свойств адаптера.

   ![Страница свойств адаптера](/platform/v5.0/administration/infrastructure/img/adapter_properties.png)
...
9. После компиляции просмотрите **журнал компиляции**.
   ...
   ![Пример журнала компиляции](/platform/v5.0/administration/infrastructure/img/adapter_compilation_log.png)

   Пример журнала компиляции
10. При необходимости скачайте архив с журналами работы адаптера (вида `_CBAP.adapters.20221026.logs.zip_`), нажав кнопку «**Скачать журнал работы**». См. *«[Мониторинг работы адаптера](#adapters_monitor)»*.

    ![Пример журнала работы адаптера](/platform/v5.0/administration/infrastructure/img/adapter_execution_log.png)
...
_![Создание подключения с использованием адаптера](/platform/v5.0/administration/infrastructure/img/adapter_custom_connection_create_menu.png)_

_![Создание пути передачи данных с использованием адаптера](/platform/v5.0/administration/infrastructure/img/adapters_custom_communication_route_create_menu.png)_
...
- На [странице свойств адаптера](#adapters_list_configure) скачайте архив с журналами адаптера, нажав кнопку «**Скачать журнал работы**». Этот журнал содержит следующие сведения:
  ...
  См. *«[Журналы адаптеров][logging_engine]»* и *«[Событие в адаптере][log_files_event_examples]»*.
- В свойствах **[подключения][connections]** адаптера нажмите кнопку «**Скачать журнал**». Этот журнал содержит следующие события:
  ...
  См. *«[Статус подключения][log_files_event_examples]»*
