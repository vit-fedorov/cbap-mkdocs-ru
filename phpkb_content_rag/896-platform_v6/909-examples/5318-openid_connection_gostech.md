...
- **Comindware Platform** интегрируется с одним поставщиком идентификации (например, Keycloak), а поддержка нескольких доменов и источников учётных записей настраивается вне **Comindware Platform**, в инфраструктуре служб идентификации организации (например, Keycloak, Active Directory, LDAP и т. п.).
- Пользователи могут проходить аутентификацию в **Comindware Platform** через SSO из разных доменов, согласно правилам, настроенным в Keycloak (`realm`, `identity provider`, `user federation` и т. п.).
...
1. Подготовьте следующие сведения, необходимые на последующих шагах настройки аутентификации через Keycloak:
   ...
   - `<yourHost>` — адрес сервера из [глобальной конфигурации](#openid_connection_server_address) **Comindware Platform**;
   - `<instanceName>.yml` — путь к файлу конфигурации экземпляра **Comindware Platform** (cм. *«[Пути и содержимое папок экземпляра ПО](https://kb.comindware.ru/article.php?id=5561)»*);
...
2. [Проверьте использование протокола HTTPS](#openid_connection_server_address) для доступа к **Comindware Platform**.
3. [Настройте службу Keycloak](#openid_connection_keycloak_configure).
4. [Настройте ОС Linux](#openid_connection_linux_configure) с экземпляром **Comindware Platform**.
5. [Инициализируйте **Comindware Platform**](#openid_connection_initialize) для входа через Keycloak.
...
1. Откройте страницу [«**Администрирование**» — «**Глобальная конфигурация**»](https://kb.comindware.ru/article.php?id=5619).
2. Удостоверьтесь, что **URL-адрес сервера** начинается с `https://`.

   ![Проверка адреса сервера](/platform/v6.0/administration/connections_communication_routes/authentication/img/openid_connection_check.png)
...
2. В левом меню выберите область (realm) **master** или другую существующую область.
   ...
   ![Переход к управлению областью master в Keycloak](/platform/v6.0/administration/connections_communication_routes/authentication/img/openid_manage_realms.png)
...
3. Если требуемая область (realm) отсутствует, нажмите кнопку **Create realm** (Создать область), введите имя области, например `<myKeycloakRealm>`, и нажмите кнопку **Create** (Создать).
   ...
   ![Создание области (realm) в Keycloak](/platform/v6.0/administration/connections_communication_routes/authentication/img/openid_create_new_realm.png)
...
6. На шаге **General settings**:
   ...
   - Введите «**Client ID**» (ID клиента), например `<myKeycloakClient>`.![Создание нового клиента Keycloak](/platform/v6.0/administration/connections_communication_routes/authentication/img/openid_create_new_client.png)
...
8. На шаге **Capability config** (Конфигурация функций) включите функции **Client authentication** (Аутентификация клиента) и **Direct access grants** (Прямое предоставление прав).
   ...
   ![Настройка конфигурации функций клиента Keycloak](/platform/v6.0/administration/connections_communication_routes/authentication/img/openid_create_new_client_2.png)
...
10. На шаге **Login settings** (Параметры входа) укажите:
    ...
    - **Web origins** (Исходные URI), например `https://<yourHost>/`.![Настройка параметров входа для клиента Keycloak](/platform/v6.0/administration/connections_communication_routes/authentication/img/openid_create_new_client_3.png)
...
12. На отобразившейся странице:
    ...
    - введите **Backchannel logout URL** (URL выхода через бэкенд), например `https://<yourHost>/OpenIdLogoutChallenge`.![Настройка параметров выхода для клиента Keycloak](/platform/v6.0/administration/connections_communication_routes/authentication/img/openid_logout_settings.png)

    Настройка параметров выхода для клиента Keycloak
13. Откройте вкладку **Credentials** (Учётные данные).
14. Сохраните **Client Secret** (Секрет клиента). Его потребуется указать в директиве `auth.openId.clientSecret` в [файле конфигурации Comindware Platform](#openid_connection_instance_configure).

    ![Настройка учётных данных для клиента Keycloak](/platform/v6.0/administration/connections_communication_routes/authentication/img/openid_copy_client_secret.png)
...
17. Настройте свойства пользователя:
    ...
    - Нажмите кнопку **Create** (Создать) или **Save** (Сохранить).![Создание нового пользователя Keycloak](/platform/v6.0/administration/connections_communication_routes/authentication/img/openid_create_new_user.png)
...
18. На вкладке **Credentials** (Учётные данные) нажмите кнопку **Set password** (Установить пароль).
    ...
    - Нажмите кнопку **Save** (Сохранить).![Установка временного пароля для пользователя Keycloak](/platform/v6.0/administration/connections_communication_routes/authentication/img/openid_create_new_user_set_password.png)

    Установка временного пароля для пользователя Keycloak

## Настройка конфигурации Comindware Platform

1. В файле конфигурации экземпляра **Comindware Platform** (`<instanceName>.yml`) настройте директивы по следующему образцу:
...
2. Добавьте в файл `/etc/hosts` строку:
   ...
   Здесь `xxx.xxx.xxx.xxx` — IP-адрес, `<yourHost>` адрес сервера **Comindware Platform**, указанный в [*глобальной конфигурации Comindware Platform*](#openid_connection_server_address) (без указания протокола `HTTP` или `HTTPS`).
...
1. Откройте веб-сайт экземпляра **Comindware Platform**, например `https://<yourHost>/`
...
6. На этом настройка входа через OpenID Connect завершена. Теперь пользователи смогут входить в **Comindware Platform** с использованием своих аккаунтов через Keycloak.
